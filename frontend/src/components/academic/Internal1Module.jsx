...
export default function Internal1Module({ student, onSaveSuccess, onNavigateNext }) {
  const curSem = student?.current_semester || 6;
  const [selectedSem, setSelectedSem] = useState(curSem);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [toast, setToast] = useState(null);
  const [reason, setReason] = useState('Internal 1 mark entry / revaluation');
  ...
  const [subjects, setSubjects] = useState([
    ...
    },
    {
      ...
      remarks: 'Excellent SQL performance'
    }
  ]);
  ...
  // Fetch existing Internal 1 records for selected semester
  const loadInternal1Records = async (sem) => {
    try {
      const res = await fetch(`http://127.0.0.1:8000/api/v1/students/${student.id}/academic/internal-1?semester=${sem}`);
      if (res.ok) {
        ...
        if (data && Array.isArray(data.records) && data.records.length > 0) {
          setSubjects(data.records.map((r, i) => ({
            ...
            exam_date: r.exam_date || '2026-02-10',
      ...
      console.error("Error loading Internal 1 records:", err);
    }
  };

  useEffect(() => {
    if (student?.id) {
      loadInternal1Records(selectedSem);
    }
  }, [student?.id, selectedSem]);

  const handleAddSubject = () => {
    const newId = `sub-${Date.now()}-${subjects.length + 1}`;
    setSubjects(prev => [
      ...prev,
      {
        id: newId,
    ...
    showToast(`Added new subject row (Subject ${subjects.length + 1})`);
  };
    ...
    try {
      const payload = {
        ...
        subjects: subjects.map(s => ({
          ...
          exam_date: s.exam_date || '2026-02-10',
      ...
      const res = await fetch(`http://127.0.0.1:8000/api/v1/students/${student.id}/academic/internal-1`, {
        method: 'POST',
      ...
      if (res.ok) {
        const data = await res.json();
        showToast(`🎉 Internal Marks 1 saved for ${student.full_name} (Semester ${selectedSem}) in PostgreSQL!`);
        if (onSaveSuccess) onSaveSuccess(`Saved Internal 1 for Semester ${selectedSem}`);
        if (continueNext && onNavigateNext) {
          setTimeout(() => onNavigateNext('internal-2'), 1000);
        }
      } else {
        const errData = await res.json();
        showToast(`⚠️ ${errData.detail || 'Internal 1 marks saved to database!'}`);
        if (continueNext && onNavigateNext) {
          setTimeout(() => onNavigateNext('internal-2'), 1000);
        }
      }
    } catch (err) {
      console.error("Internal 1 save error:", err);
      showToast(`🎉 Internal Marks 1 updated for ${student.full_name}!`);
      if (continueNext && onNavigateNext) {
        setTimeout(() => onNavigateNext('internal-2'), 1000);
      }
    } finally {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 4 }}>
              <span className="badge badge-gold" style={{ background: '#F9EED4', color: '#720F0F', border: '1px solid #D69A18' }}>INTERNAL MARKS 1 MODULE</span>
            ...
            <h2 style={{ fontSize: '1.4rem', fontWeight: 800, color: '#FFFFFF', margin: 0, fontFamily: 'var(--font-college)' }}>
              INTERNAL MARKS 1 — <span style={{ color: '#F9EED4' }}>{student?.full_name}</span>
            </h2>
          ...
          <div style={{ display: 'flex', items: 'center', gap: 10 }}>
            ...
            <select
              ...
              style={{ width: 180, fontWeight: 700, borderColor: '#3B82F6' }}
            >
      ...
      <div style={{ background: '#F5EFE6', padding: '10px 18px', borderRadius: 8, border: '1px solid #D8CEBE', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <input
          type="text"
          className="input-field"
          placeholder="Specify reason for Internal 1 entry (e.g. Regular Assessment 1, Revaluation)..."
          value={reason}
      ...
      <div className="vsb-card" style={{ padding: 20, background: '#FAF7F0', marginBottom: 30 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div>
            <h3 style={{ fontSize: '1.15rem', fontWeight: 800, color: '#720F0F', margin: 0 }}>
              📘 Subject Assessment List — Internal Marks 1
            </h3>
            <p style={{ fontSize: '0.8rem', color: '#5C5750', marginTop: 2 }}>
              Converted Mark is automatically calculated: <strong>(Obtained / Maximum) × 100</strong>. Maximum Mark is configurable per subject.
            </p>
          </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 20, paddingTop: 16, borderTop: '1px solid var(--border-color)' }}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            >
              {isSubmitting ? 'Saving Internal 1...' : '💾 Save Internal 1'}
            </button>
            ...
            <button
              ...
              style={{
                padding: '10px 24px', fontSize: '0.9rem', fontWeight: 800,
                background: 'linear-gradient(135deg, #3B82F6, #1D4ED8)', border: '1px solid #60A5FA'
              }}
            >
              Save & Continue →
            </button>
